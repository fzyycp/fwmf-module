...
public interface UserInfoMapper {
    ...
    @Select("SELECT USER_ID userId,LOGIN_NAME loginName,USER_NAME userName,`PASSWORD` `password`,EFCT_YMD efctYmd," +
            ...
            "  FROM " + DBConstsOfUserRole.TN_USER_INFO
     ...
     * @return 用户ID
     */
    @Insert("INSERT INTO " + DBConstsOfUserRole.TN_USER_INFO
    ...
    @Update("UPDATE " + DBConstsOfUserRole.TN_USER_INFO + " SET USER_NAME=#{userName},EFCT_YMD=#{efctYmd},EXPR_YMD=#{exprYmd},UPDATE_PERSON=#{updatePerson},UPDATE_TIME=CURRENT_TIMESTAMP WHERE USER_ID=#{userId}")
    int updateUserInfoById(Map<String, Object> map);

    @Update("UPDATE " + DBConstsOfUserRole.TN_USER_INFO + " SET IS_DELETE='1',UPDATE_PERSON=#{updatePerson},UPDATE_TIME=CURRENT_TIMESTAMP WHERE USER_ID=#{userId}")
    int deleteUserInfoById(Map<String, Object> map);

    @Update("UPDATE " + DBConstsOfUserRole.TN_USER_INFO + " SET IS_ENABLE=#{isEnable},UPDATE_PERSON=#{updatePerson},UPDATE_TIME=CURRENT_TIMESTAMP WHERE USER_ID=#{userId}")
    int changeEnable(Map<String, Object> map);

    @Update("UPDATE " + DBConstsOfUserRole.TN_USER_INFO + " SET `PASSWORD`=#{password},UPDATE_PERSON=#{updatePerson},UPDATE_TIME=CURRENT_TIMESTAMP WHERE USER_ID=#{userId}")
    int resetPassword(Map<String, Object> map);

    @Update("UPDATE " + DBConstsOfUserRole.TN_USER_INFO + " SET `PASSWORD`=#{newPassword},UPDATE_PERSON=#{updatePerson},UPDATE_TIME=CURRENT_TIMESTAMP WHERE USER_ID=#{userId} AND `PASSWORD`=#{oldPassword}")
